...
def get_pf_map(paragraph_words):
    pf = Counter()
...
def main():
    ...
    for qid in bl_query_db.keys():
        ...
        pf_map  =  get_pf_map(paragraph_words)
        # sys.exit()
        doc_vector =  doc_vector_generator.get_doc_vector(pf_map)
        ...
        for word in pf_map:
            ...
            all_pf.append(pf_map[word])


        query_dest_file = os.path.join(meausre_dir, '{}.png'.format(qid))

        plt.plot(query_pf, query_measure, 'ro')
